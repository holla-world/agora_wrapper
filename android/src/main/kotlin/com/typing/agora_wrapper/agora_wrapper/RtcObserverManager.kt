...
class RtcObserverManager() {
    ...
    fun onMethodCall(@NonNull call: MethodCall, @NonNull result: MethodChannel.Result): Boolean {
        when (call.method) {
...
//            Log.e(
//                TAG,
//                "updateVideoFrameBuffer: render cost more than 2000ms a frame, what a amazing device!"
//            )
        }
        outputData?.image?.buffer?.let { buffer ->
            videoFrame.apply {
                ...
                System.arraycopy(
                    buffer,
                    getyBuffer().size + getuBuffer().size,
                    getvBuffer(),
                    0,
                    getvBuffer().size
                )
